...
public class ClubRepositoryImpl extends SimpleMongoRepository<ClubDoc, Integer> implements ClubRepositoryCustom{
	...
	public void addClubMember(ClubMemberDoc clubMember) {
		...
		ClubMember member = new ClubMember(clubMember.getClubNo(), clubMember.createDomain().getRolePerson());
	...
	@Override
	public void deleteClubMemberByEmail(int clubNo, String email) {
		//
		MongoOperations operations = getMongoOperations();
	...
	@Override
	public void updateClubManager(ClubManagerDoc clubManagerDoc) {
		//
		MongoOperations operations = getMongoOperations();
	      
		String email = clubManagerDoc.getRolePerson().getEmail();
		Criteria criteria = Criteria.where("_id").is(clubManagerDoc.getClubNo());
		...
		Update update = new Update();
		update.set("managers.$", clubManagerDoc);
